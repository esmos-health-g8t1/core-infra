...
```mermaid
graph TD
    subgraph Azure_Cloud["Azure Cloud (Southeast Asia)"]
        ...
        subgraph Ops["Observability"]
            ...
            Grafana["Azure Managed Grafana (Manually Provisioned)"]
        end
...
| **Secrets** | `azurerm_key_vault` | Zero-manual-secret policy via Terraform auto-population (17 secrets). |
| **Monitoring**| `azurerm_log_analytics_workspace`| Centralized log aggregation. (Managed Grafana also enabled manually). |
...
cd core-infra/azure
terraform init \
  -backend-config="resource_group_name=esmos-healthcare-rg" \
...
### 3. Verification
Once deployed, check the **Outputs** for the Key Vault name. Verify all 17 secrets are present:
```bash
az keyvault secret list --vault-name <kv-name> -o table
```

Deploy GCP edge layer next — see [`../gcp/`](../gcp/).
...
> **Edge layer**: Handled by GCP Global Load Balancer — see [`../gcp/`](../gcp/).
